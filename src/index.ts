...
var rewardSlots = ["first_item", "second_item", "third_item", "fourth_item", "fifth_item", 
					...
					"fifteenth_item", "sixteenth_item", "seventeenth_item", "eighteenth_item",
...
export async function initOnLoad() {
	...
	toggleLootDisplay("artifacts_rewards")
	toggleLootDisplay("materials_rewards")
	toggleLootDisplay("miscellaneous_rewards")
	await init();
...
function tabChanger(){
	...
	for(let i = 0; i < keys.length; i++){
		items[keys[i]].tab = lsdb[keys[i]].tab
	}
}
...
export async function init() {
	...
	// TODO: This is a fix for when the tab buttons are clicked once.
...
async function addHistoryToLs(value: number, item: any, quants: any, reward: any) {
	...
	let previous = [itemsArr, quants, value, reward, localStorage.getItem("TetraLogger/Count"), localStorage.getItem("TetraLogger/PrimaryKeyHistory"), currentDateTime];
		...
		for (let i = lsHistory.length - 1; i >= 0 ; i--) { //Navigating lsHistory
			if (limit < parseInt(localStorage.getItem("TetraLogger/HistoryDisplayLimit"))) {
				...
				if (temp[3].includes(" [C] ")) {
					...
					let countText = "Casket reward: " + index;
					...
					let count = document.createElement("div") as HTMLDivElement;
					count.textContent = "Casket reward: " + index;
					count.setAttribute('class', 'historyCount');
					container.append(count);
				}
...
export async function verifyInsert(event: Event) {
	...
	let countText = "ancient casket" + ": " + curr;
	let count = document.createElement("div") as HTMLDivElement;
	count.innerHTML = countText;
	count.setAttribute('class','historyCount');
	count.append(customSpan);
...
export function insertToDB() {
	...
	if (window.alt1) {
		alt1.overLayClearGroup("overlays");
		alt1.overLaySetGroup("overlays");
		alt1.overLayTextEx("Custom Tetracompass casket submitted successfully!",
			a1lib.mixColor(100, 255, 100), 20, Math.round(alt1.rsWidth / 2), 200, 4000, "", true, true);
	}
}
...
export function exporttocsv() {
	...
	csvinfo.push(["Rewards Tier & Count", "Reward Value", "Date and Time recorded", "Item 1", "Item 2", "Item 3", "Item 4", "Item 5", "Item 6", "Item 7", "Item 8", "Item 9", "Item 10", "Item 11", "Item 12", "Item 13", "Item 14", "Item 15", "Item 16", "Item 17", "Item 18", "Item 19", "Item 20", "Item 21"])
	...
	let filename = "TetraLogger CSV " + (d.getFullYear() + "-" + month.slice(-2) + "-" + day.slice(-2) + "--" + hour.slice(-2) + "-" + minute.slice(-2) + "-" + second.slice(-2)) + ".csv";
...
function imgMaker(item: string, quant:number) {
	...
	if(["Golden dragonfruit seed", "Carambola seed"].includes(item)){
		if(quant == 3)
...
export function toggleLootDisplay(id: string) {
	...
	else if (id == "miscellaneous_rewards") {
	...
	if (opentabs[0]) {
		Array.from(document.getElementsByClassName('artifacts') as HTMLCollectionOf<HTMLElement>)[0].style.minHeight = minHval;
	}
	else {
		Array.from(document.getElementsByClassName('artifacts') as HTMLCollectionOf<HTMLElement>)[0].style.minHeight = "8%";
	}

	if (opentabs[1]) {
		Array.from(document.getElementsByClassName('materials') as HTMLCollectionOf<HTMLElement>)[0].style.minHeight = minHval;
	}
	else {
		Array.from(document.getElementsByClassName('materials') as HTMLCollectionOf<HTMLElement>)[0].style.minHeight = "8%";
	}

	if (opentabs[2]) {
		Array.from(document.getElementsByClassName('miscellaneous') as HTMLCollectionOf<HTMLElement>)[0].style.minHeight = minHval;
	}
	else {
		Array.from(document.getElementsByClassName('miscellaneous') as HTMLCollectionOf<HTMLElement>)[0].style.minHeight = "8%";
	}
}
